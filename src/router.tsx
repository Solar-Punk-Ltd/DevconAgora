...
const MainRouter = (): ReactElement => {
  ...
  const fetchFeedUpdate = useCallback(async () => {
    if (isBeeRunning) {
      ...
      const ref = await getFeedUpdate(process.env.FEED_OWNER_ADDRESS as string, rawFeedTopicSession);
  ...
  const fetchSessions = useCallback(async () => {
    if (sessionsReference.length === ADDRESS_HEX_LENGTH) {
      ...
      try {
        ...
        console.log(`error parsing session, ref ${sessionsReference}:\n ${error}`);
        return;
      }
    ...
    try {
      ...
      for (let i = 0; i < recentSessions.length; i++) {
        ...
        commentPromises.push(
          loadLatestComments(stamp, rawTalkTopic, signer, process.env.BEE_API_URL, MAX_COMMENTS_LOADED),
        );
  ...
  const calcActivity = () => {
    if (loadedTalks) {
      ...
      for (let i = 0; i < recentSessions.length; i++) {
        ...
        if (foundIx > -1) {
          tmpActiveVisitors.set(recentSessions[i].id, loadedTalks[foundIx].nextIndex);
        }
      }
    ...
    <>
      ...
      <Routes>
        ...
        <Route
          ...
          element={<HowDoesItWork toText={prevLocation ? prevLocation : undefined} />}
        />
